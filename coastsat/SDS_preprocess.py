...
def preprocess_single(fn, satname, cloud_mask_issue):
    ...
    if satname == 'L5':
        ...
        bands = [data.GetRasterBand(k + 1).ReadAsArray()
                 for k in range(data.RasterCount)]
    ...
    if satname == 'S2':
        ...
        im_zeros = np.logical_and(
            np.isin(im_ms[:, :, 1], 0), im_zeros)  # Green
    ...
    if sum(sum(cloud_mask)) > 0 and sum(sum(~cloud_mask)) > 0:
        ...
        if cloud_mask_issue:
            ...
            cloud_mask = morphology.binary_opening(
                cloud_mask, elem)  # perform image opening
            # remove objects with less than 25 connected pixels
            morphology.remove_small_objects(
                cloud_mask, min_size=25, connectivity=1, in_place=True)
...
def pansharpen(im_ms, im_pan, cloud_mask):
    ...
    im_ms_ps = vec_ms_ps_full.reshape(
        im_ms.shape[0], im_ms.shape[1], im_ms.shape[2])
...
def save_jpg(metadata, settings, **kwargs):
    ...
    filepath_jpg = os.path.join(
        filepath_data, sitename, 'jpg_files', 'preprocessed')
...
def get_reference_sl(metadata, settings):
    ...
    # otherwise get the user to manually digitise a shoreline on S2, L8 or L5 images (no L7 because of scan line error)
        ...
        for i in range(len(filenames)):
            ...
            # rescale image intensity for display purposes
            im_RGB = rescale_image_intensity(
                ...
                while 1:
                    ...
                    if pt_input[0][0] > im_ms.shape[1]/2:
                        ...
                        plt.title('Reference shoreline saved as ' + sitename +
                                  '_reference_shoreline.pkl and ' + sitename + '_reference_shoreline.geojson')
                        plt.draw()

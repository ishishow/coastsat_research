...
settings = {
    ...
    # epsg code of spatial reference system desired for the output
    'output_epsg': 3857,
    # quality control:
    # if True, shows each shoreline detection to the user for validation
    'check_detection': False,
    # if True, allows user to adjust the postion of each shoreline by changing the threhold
    'adjust_detection': False,
    # if True, saves a figure showing the mapped shoreline for each image
    'save_figure': True,
    # [ONLY FOR ADVANCED USERS] shoreline detection parameters:
    # minimum area (in metres^2) for an object to be labelled as a beach
    'min_beach_area': 4500,
    # radius (in metres) of the buffer around sandy pixels considered in the shoreline detection
    'buffer_size': 150,
...
for i, key in enumerate(cross_distance.keys()):
    ...
    ax.set_ylim([-50, 50])
    ax.plot(output['dates'], cross_distance[key] -
            np.nanmedian(cross_distance[key]), '-o', ms=6, mfc='w')
...
ax.plot(tide_data['dates'], tide_data['tide'],
        '-', color='0.6', label='all time-series')
ax.plot(dates_sat, tides_sat, '-o', color='k', ms=6,
        mfc='w', lw=1, label='image acquisition')
ax.set(ylabel='tide level [m]', xlim=[dates_sat[0], dates_sat[-1]],
...
for i, key in enumerate(cross_distance.keys()):
    ...
    ax.set_ylim([-50, 50])
    ax.plot(output['dates'], cross_distance[key] -
            np.nanmedian(cross_distance[key]), '-o', ms=6, mfc='w', label='raw')
    ax.plot(output['dates'], cross_distance_tidally_corrected[key] - np.nanmedian(
        cross_distance[key]), '-o', ms=6, mfc='w', label='tidally-corrected')
    ax.set_ylabel('distance [m]', fontsize=12)
    ax.text(0.5, 0.95, key, bbox=dict(boxstyle="square", ec='k', fc='w'), ha='center',
            va='top', transform=ax.transAxes, fontsize=14)
ax.legend()
